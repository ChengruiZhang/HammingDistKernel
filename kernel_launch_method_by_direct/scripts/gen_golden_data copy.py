...
def hamming_topk_pipeline_bitpacked(
    ...
    q_bits01 = unpack_bits_uint16(qhash_packed_u8, hid_dim, msb_first=True)
    k_bits01 = unpack_bits_uint16(khash_packed_u8, hid_dim, msb_first=True)
    ...
    if input_save_dir is not None:
        ...
        # topK 索引：int32
        save_bin(os.path.join(output_save_dir, "golden_topk_value.bin"), topk_vals.astype(np.int32))
...
def unpack_bits_uint16(x: np.ndarray, hid_dim: int, msb_first: bool=True) -> np.ndarray:
    if x.dtype != np.uint16:
        raise ValueError(f"expect uint16 packed array, got {x.dtype}")
    *prefix, pack = x.shape
    shifts = np.arange(15, -1, -1, dtype=np.int32) if msb_first else np.arange(0, 16, dtype=np.int32)
    y16 = ((x[..., None] >> shifts) & 1).astype(np.int8)    # [..., pack, 16]
    y = y16.reshape(*prefix, pack * 16)
    return y[..., :hid_dim]
